...
function AIdialogBox({ show }) {
  ...
  const convertTextToHTML = (rawText) => {
      ...
      // 使用正则表达式匹配代码块，并添加<code>标签
      formattedText = formattedText.replace(/`([^`]+)`/g, '<code>$1</code>');

      // 将列表标记（数字或点号）替换为HTML列表项
      formattedText = formattedText.replace(/(\d+\.)\s/g, '<li>$1 ').replace(/•\s/g, '<li>');

      // 添加<ol>和<ul>标签
      formattedText = formattedText.replace(/^<li>/, '<ol><li>') + '</ol>';
  ...
  return (
    <div data-blur_close_key="CHANGE_AIDIALOGBOX_SHOW" style={{display: show ? 'block' : 'none', background: backgroundColor_1}} className='aidialog_content'>
      ...
      <div style={{ background: backgroundColor_2 }} className='show_message_class hiddeScroll'>
        {messages.map((m, i) => {
          ...
          return (
            <div key={i} className='assistant_message_content'>
              <div dangerouslySetInnerHTML={{ __html: convertTextToHTML(m.content) }} style={{ background: backgroundColor_1, color: textColor }}>
              </div>
            </div>
          )
        })}
        ...
        {currentResContent ? (
          <div id="assistant_message" className='assistant_message_content'>
            <div dangerouslySetInnerHTML={{ __html: convertTextToHTML(currentResContent) }} style={{ background: backgroundColor_1, color: textColor }}>
            </div>
          </div>
        ) : ''}
        {loading ? (
      ...
      <div style={{ background: backgroundColor_2 }} className='input_message'>
        <textarea value={content} placeholder='你可以问我任何问题' onChange={onTextAreaChange} className='textarea-class' id="story" name="story" rows="5" cols="33" />
